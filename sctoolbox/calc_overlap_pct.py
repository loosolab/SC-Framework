...
#     merged_ov_dict = mp_counter_2(ov_fragments, barcodes=barcodes, column=col_n_fragments_in_list)
#     fragments = pd.read_csv(fragments_file, sep="\t", header=None, chunksize=1000000)
#     merged_fl_dict = mp_counter_2(fragments, barcodes=barcodes, column=col_total_fragments)
#
#     # add to adata.obs
...
#     if merged_dict:
#         merged_dict = dict(Counter(merged_dict) + Counter(result))
#     else:
#         merged_dict = result
#
#
# def mp_counter_2(fragments, barcodes, column, n_threads=8):
#
#     pool = mp.Pool(n_threads, maxtasksperchild=48)
#     jobs = []
#     for chunk in fragments:
#         # print(chunk[:][0], barcodes[0], column)
#         job = pool.apply_async(get_barcodes_sum, args=(chunk, barcodes, column), callback=log_result)
#         jobs.append(job)
#     pool.close()
#     pool.join()
#
