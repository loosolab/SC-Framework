# <Title>

## Problem

<What are we trying to do, and why now? Reference related prior work if relevant.>

## Approach

<High-level idea, not implementation. Which sctoolbox modules / data flow it
touches, the angle, and what we deliberately will NOT do.>

## Scope

- Type: one or more of package | notebooks | docs  (combine with "+", e.g. package + docs)
- Conda environment: <name>
- Commit mode: manual | claude (Name <email>)  (manual = user stages + commits; claude = Claude commits with this identity as author + committer)
- Autonomy: per-task | end  (per-task = /implement pauses for review/commit after each task; end = run all tasks, then review once)

## Success criteria

- <Concrete, observable signal — numeric where possible>
- <Add one per distinct outcome>

## Open questions

- <What the user must decide before planning — empty if all resolved>
